...
// Después, borrando también sus notas:

app.delete('/user/:id', async (req, res) => {
    const token = req.headers['authorization'];
    if (!token) return res.status(403).json({ message: 'No autenticado.' });

    // Verificamos que quien llama sea admin
    ...
    // 3) Borrar todas sus notas
    const resultado = await Note.deleteMany({ usuarioId: req.params.id });


    // 4) Responder al cliente
    res.json({ message: 'Usuario y notas asociadas eliminados.' });
});
